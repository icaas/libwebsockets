...
task(void *user, enum lws_threadpool_task_status s)
{
	struct task_data *priv = (struct task_data *)user;
	int budget = 100 * 1000;
			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_PROTOCOL_INIT:
		...
		cargs.name = "tp example";
		cargs.max_queue_depth = 8;
		cargs.threads = 3;
		vhd->tp = lws_threadpool_create(&cargs);
		if (!vhd->tp)
			return 1;
		break;
	...
	case LWS_CALLBACK_ESTABLISHED:
		...
		pss->priv = args.user = create_task_private_data();
		if (!args.user)
			return 1;

		pss->priv->pos = 0;
		pss->priv->end = 10 * 1000 * 1000;
		...
		pss->tp_task = lws_threadpool_enqueue(vhd->tp, &args);
		if (!pss->tp_task) {
			lwsl_user("%s: Couldn't enqueue task\n", __func__);
			cleanup_task_private_data(wsi, pss->priv);
			return 1;
		}
	...
	case LWS_CALLBACK_WS_SERVER_DROP_PROTOCOL:
		...
		if (pss && pss->tp_task) {
			lws_threadpool_dequeue(pss->tp_task);
			pss->tp_task = NULL;
		}
		break;
	...
	case LWS_CALLBACK_SERVER_WRITEABLE:
		...
		n = lws_threadpool_task_status(pss->tp_task);
		...
		case LWS_TP_STATUS_STOPPED:
			/* task has actually been freed (and cleaned up / freed
			 * its args.user)... kill our now invalid pointers to
			 * the task and the task data */
			pss->tp_task = NULL;
			pss->priv = NULL;
			/* close the wsi */
			return -1;
		...
		n = strlen(pss->priv->result + LWS_PRE);
		m = lws_write(wsi, (unsigned char *)pss->priv->result + LWS_PRE,
			      n, LWS_WRITE_TEXT);
		if (m < n) {
			lwsl_err("ERROR %d writing to ws socket\n", m);
			lws_threadpool_task_sync(pss->tp_task, 1);
			return -1;
		}
		 ...
		 * continue now.
		 */
		lws_threadpool_task_sync(pss->tp_task, 0);
		break;
